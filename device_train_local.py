...
def save(network, step, bucket, path, mp, aux=None, keep_n=3, delete_old=True):
    ...
    try:
        with open(f"{bucket}/{path}/meta.json", "r") as f:
            meta = json.load(f)
    except:
        # create metadata file
        with open(f"{bucket}/{path}/meta.json", "w") as f:
            json.dump({
                "step": 0,
                "checkpoints": [],
                "aux": {}
    ...
    for shard_id in range(mp):
        write_ckpt(network.state, f"{bucket}/{path}/step_{step}/", shard_id)
...
if __name__ == "__main__":
    ...
    if args.tune_model_path:
        ...
        initial_ckpt_path = f"{bucket}/{initial_ckpt_model_dir}"
